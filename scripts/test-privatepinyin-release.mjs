...
assert.match(productData, /latestVersion: '0\.1\.25'/);
assert.match(productData, /minimumSystem: 'macOS 14 or later'/);
assert.match(productData, /PrivatePinyin-0\.1\.25\.pkg/);
assert.match(productData, /399d72d4a715aeea8bceb09dd7ebe238ecf3d6a3fe72920965b37ccaa33c5ac1/);
...
assert.match(worker, /privatepinyin\/0\.1\.25\/PrivatePinyin-0\.1\.25\.pkg/);
...
assert.equal(stableManifest.version, '0.1.25');
assert.equal(stableManifest.build, 25);
...
assert.equal(stableManifest.package_url, 'https://wwwstationcat.org/downloads/privatepinyin/PrivatePinyin-0.1.25.pkg');
assert.equal(stableManifest.package_sha256, '399d72d4a715aeea8bceb09dd7ebe238ecf3d6a3fe72920965b37ccaa33c5ac1');
assert.equal(stableManifest.package_size_bytes, 14053238);
...
assert.match(productPage, /新增 Writer V1/);
...
assert.match(downloadPage, /macOS \$\{macVersion\} 新增 Writer V1/);
assert.match(downloadPage, /約 1\.1 GB/);
assert.match(downloadPage, /嚴格隱私模式會停用並取消 Writer/);
