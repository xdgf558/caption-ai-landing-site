...
assert.match(productData, /latestVersion: '0\.1\.27'/);
assert.match(productData, /minimumSystem: 'macOS 14 or later'/);
assert.match(productData, /PrivatePinyin-0\.1\.27\.pkg/);
assert.match(productData, /00eca727600f37476e1676207c0307bf685d4883b3b8f6be63cb6e56216d16bf/);
...
assert.match(worker, /privatepinyin\/0\.1\.27\/PrivatePinyin-0\.1\.27\.pkg/);
...
assert.equal(stableManifest.version, '0.1.27');
assert.equal(stableManifest.build, 27);
...
assert.equal(stableManifest.package_url, 'https://wwwstationcat.org/downloads/privatepinyin/PrivatePinyin-0.1.27.pkg');
assert.equal(stableManifest.package_sha256, '00eca727600f37476e1676207c0307bf685d4883b3b8f6be63cb6e56216d16bf');
assert.equal(stableManifest.package_size_bytes, 14072621);
...
assert.match(productPage, /偏好設定改為緊湊的一級總覽與二級功能頁/);
...
assert.match(downloadPage, /macOS \$\{macVersion\} 將偏好設定整理為緊湊的一級總覽/);
assert.match(downloadPage, /視窗也繼續支援等比例縮放/);
assert.match(downloadPage, /Writer V1 和嚴格隱私保護也繼續提供/);
