...
assert.match(pageSource, /一份餘額，通用於整個 Station Cat/);
assert.match(pageSource, /One balance across Station Cat/);
assert.match(pageSource, /getVipMembershipCopy\(lang, musicEntryEnabled\)/);
assert.match(pageSource, /title: vip\.name, body: vip\.readingDescription/);
assert.match(pageSource, /plannedHeading: vip\.plannedHeading/);
assert.match(pageSource, /title: vip\.music, body: vip\.musicDescription/);
assert.match(pageSource, /title: vip\.software, body: vip\.softwareDescription/);
assert.doesNotMatch(pageSource, /付費軟體下載權限|Paid software downloads/);
assert.equal((pageSource.match(/useIntro: vip\.useIntro/g) || []).length, 4);
assert.equal((pageSource.match(/redemptionBody: vip\.redemptionBody/g) || []).length, 4);
assert.doesNotMatch(pageSource, /只有顯示積分價格與兌換按鈕|only when it displays a points price/);
assert.match(pageSource, /data-state="active"/);
assert.match(pageSource, /data-state="planned"/);
...
assert.match(librarySource, /查看價格與完整積分規則/);
assert.doesNotMatch(librarySource, /100 Station 積分 · 10 美元/);
assert.match(librarySource, /Station Cat 全站通用積分/);
assert.match(librarySource, /getVipMembershipCopy\(lang, musicEntryEnabled\)/);
assert.match(librarySource, /\{vip\.notice\}/);
