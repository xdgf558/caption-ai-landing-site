...
assert.match(pageSource, /title: vip\.name, body: vip\.intro/);
...
assert.match(pageSource, /規劃中的用途目前不能兌換/);
assert.match(pageSource, /Planned uses cannot be redeemed yet/);
...
assert.match(librarySource, /目前已開放/);
assert.match(librarySource, /付費軟體下載權限/);
assert.match(librarySource, /規劃中的用途目前還不能兌換/);
