...
for (const language of ['zh-CN', 'en', 'ja']) test(language + ': latest only; all archived notes retained and collapsed', () => {
  ...
  assert.deepEqual(Array.from(game.config.releaseHistory, item => item.version), ['1.26.2', '1.26.1', '1.26.0', '1.25.1', '1.25.0', '1.24.0', '1.23.0', '1.22.2']);
  assert.deepEqual(Array.from(game.config.releaseHistory, item => item.notes[language].length), [3, 3, 3, 2, 3, 3, 4, 1]);
