...
test('P3: 1.26.0 release version and three-language notes appear for a 1.24.0 save', () => {
  fresh();
  const manifestContext = { window: {} };
  vm.runInNewContext(readFileSync(new URL('content-manifest.js', root), 'utf8'), manifestContext);
  assert.equal(game.config.version, '1.26.0');
  assert.equal(manifestContext.window.CatGameContentManifest.manifest.releaseVersion, game.config.version);
  assert.match(readFileSync(new URL('../src/data/products/cat-life-game.ts', import.meta.url), 'utf8'), /latestVersion: '1\.26\.0'/);
