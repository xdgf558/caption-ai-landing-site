...
Add new assets under `public/games/cat-life/src/assets/premium/`, declare their intrinsic dimensions and conditions in the manifest, and update the relevant renderer without duplicating entitlement keys or asset paths elsewhere. Add the product page media only after the final asset exists. Every content change must update the public game version in `src/js/core/namespace.js`. Before replacing `releaseNotes`, archive the previous release's version and complete zh-CN/en/ja notes in `releaseHistory` (newest first, without duplicate version entries). Then write only the new release's changes into localized `releaseNotes`; do not append historical changes to the current notes or delete earlier history. Keep the manifest's `releaseVersion`, product `latestVersion`, and release assertions aligned with the new game version. The version page will display the latest notes and collapse historical entries by default.
