...
- Each care-date stage has one package: basic food ×2, litter ×1 and toy uses ×2. Claims are persisted and cannot be repeated through navigation, reload, import, or disabled auto-save.
...
- `scripts/test-cat-life-onboarding.mjs`: action semantics, persistence, UTC dates, time away, rollback, zero-resource recovery, free treatment, old/future saves, multi-cat targeting, three-language rendering, and no-op protection.
- `scripts/browser-tests/cat-life-onboarding.spec.mjs`: the full first session, reload deduplication, seven-day return, clinic aid, UTC graduation, responsive layouts, and all supported languages.
