...
| 权利策略 | 免费曲、试听及 VIP 曲均须满足本站商业运营使用权核对门槛 |
...
每首作品都需要记录来源和人工核对结论。系统只检查材料和状态是否齐备，不宣称能够判断作品绝对不存在第三方权利争议。

| 作品情况 | 系统处理 |
| :--- | :--- |
| 有适用授权、合规下载记录、素材权利说明 | 管理员完成核对后可发布 |
| 生成或下载套餐不清楚 | 保持待核对状态 |
| 免费期作品，只有后来订阅的凭证 | 不自动批准，补充适用条款或 Suno 书面确认 |
| 使用他人歌词、采样、翻唱素材 | 单独补充相应权利依据 |
| 使用 Suno 特定 Remix 功能所得作品 | 默认阻止公开首发，获得明确适用许可后人工复核 |
| 只有 Suno 页面链接或临时媒体地址 | 不能视为本站上传所需的文件来源证明 |
| 存在投诉或明显冒充、侵权疑点 | 先下架，记录原因，再核对 |

站长已经合法取得的历史文件应保留取得时间和当时凭证；不要仅为填新字段就推断其权利失效，亦不要自行承诺旧作品已获得新授权。
...
首版公开发布门槛明确为 `reviewStatus=approved`、`permittedUse=commercial`、`downloadMethod=official`，并且 `authorizationBasis` 已填写且证据材料经管理员核对。套餐名称本身不能自动生成 approved。对 Remix 或旧作品存在例外许可的情况，须在依据中记录具体适用文件并人工确认商业使用资格；不能仅填一个普通备注绕过 blocked。该门槛属于本站首发风险控制选择，免费作品的单独非商业展示需要另行设计适用规则。

审批绑定具体版本。更换音频、歌词或影响权利判断的内容后，旧审批失效，新版本重新核对。试听片段必须关联同一已审批完整音频，保留合法取得的源文件和裁剪过程，不因剪成短片段而跳过权利审核。
...
7. 管理员选择 free/vip/early_access，按需上传独立试听并填写起止位置；试听完整和短文件，填写资料及权利核对。
...
以下任一条件成立则禁止发布，并在后台显示具体字段或原因：缺少有效音频、音频超规格、封面不合规、标题为空、草稿版本冲突、资源归属不符、技术试听未确认、未满足第 3.3 节全部权利门槛、已发布数量超上限、访问策略未确认或非法、抢先规则不完整、VIP/early_access 缺少有效独立试听、试听超过时长上限或与当前源音频不匹配。
...
P0 功能完成，全部权限和发布一致性测试通过，关键 Range 用例通过，真实 iPhone 主要收听流程通过，现站回归通过，备份和回滚演练有记录，真实作品权利材料经过人工核对、全站会员映射和退款/撤销依赖已明确、媒体缓存隔离与试听强校验通过、会员说明与售卖权益一致，才可判定首发具备上线条件。
