...
`test:music:rate-limits` 11/11、公开接口 9/9、媒体 10/10、原生 D1/R2 runtime 25/25。覆盖来源/全局竞争限额、拒绝不占全局、分钟/类别隔离、IPv6 规范化、无原 IP 的 HMAC、配置/数据库/超时失败关闭、全局计数故障整体回滚、分批过期计数回收、HEAD/Range/匿名展示入口前置限流、总闸零访问及诊断 Access/只读/脱敏。

PR #134 清理的远端 CI 已通过；本批 CI 随第二个草稿 PR 运行。真实云端跨实例、Cloudflare 来源头、延迟/成本和阈值压测仍未验收。
