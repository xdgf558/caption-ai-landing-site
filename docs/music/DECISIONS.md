...
## 2026-09-13：M6-04 限于预发运维，生产资源移至 M6-05

用户明确将生产音乐资源准备留到 M6-05，先完成隔离预发运维验收。离机备份使用指定 Google Drive 账号的私有目录，保留 30 天，采用登记到期日、到期提醒和人工确认删除；它不是自动删除保证。生产资源/迁移/配额、生产监控与备份目标、部署和业务开闸仍逐项批准。实际账号、预算通知邮箱、备份地址及全部执行材料仅保存在仓库外，PR 只交付实现、合同与任务状态。见 [运维合同](../../ops/MUSIC_OPERATIONS_READY.md)。

## 2026-09-13：整个音乐后台来源材料可选

用户明确取消来源材料前置条件，适用于整个音乐后台。未提供或 pending 不阻止上传、技术核对或显式发布，不自动生成 approved；明确 blocked 仍阻止技术核对/发布，主动提交 approved 仍要求原有完整依据。实际试听、资源验证、独立试听、VIP、配额与审计不变。此决定取代下方历史“权利与技术双门槛”的必填材料部分；见 [来源材料可选合同](OPTIONAL_SOURCE_MATERIALS.md)。实施不附带部署、开闸或正式作品发布，执行/session 材料仍不上传。
