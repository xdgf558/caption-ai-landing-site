...
日期：2026-09-09。M0 只核对和规划，以下拟新增部分均未实现。完整要求以 [原规格](DEVELOPMENT_SPEC.md) 为准。
...
2026-09-09 后续确认：PR #118 已上线，用户报告真实兑换成功。退款采用人工审核：冲正进入待审核队列，确认资金关联后只撤销对应兑换未使用的期限；保留审计，不按负积分自动取消全部 VIP。无回执或无法验证的历史记录继续人工核对。实现作为原会员独立变更，详见 [退款人工审核](../membership-refund-review.md)，未批准部署或音乐开闸。
