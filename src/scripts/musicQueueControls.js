...
export function mountMusicQueueControls(root, { queue, player, getVisibleTracks }) {
  ...
  const render = state => {
    ...
    text('[data-repeat-label]', repeatText[state.repeat]);
    $('[data-repeat]').setAttribute('aria-label', `${repeatText[state.repeat]}，点击切换循环方式`);
    ...
    for (const selector of ['[data-queue-notice]', '[data-dock-queue-notice]']) {
      text(selector, state.notice?.message || ''); $(selector).hidden = !state.notice;
    }
    $('[data-queue-empty]').hidden = state.items.length > 0;
    $('[data-queue-clear]').disabled = !state.items.length;
    const selected = getVisibleTracks().find(track => track.id === state.activeTrackId);
    ...
    for (const [index, track] of state.items.entries()) {
      ...
      row.querySelector('[data-queue-meta]').textContent = `${index + 1} · ${!track.available ? '暂不可用' : track.canPlayFull ? '完整收听' : track.previewAvailable ? '仅手动试听' : '暂无试听'}${current ? ' · 当前曲目' : ''}`;
      ...
      play.setAttribute('aria-label', `${current && ['loading', 'playing', 'buffering'].includes(player.snapshot().status) ? '暂停' : '播放'}队列曲目：${track.title}`);
      row.querySelector('[data-queue-remove]').setAttribute('aria-label', `移出队列：${track.title}`);
    }
  ...
  on(list, 'click', event => {
    ...
    if (button.hasAttribute('data-queue-play')) queue.playQueued(row.dataset.queueId);
      ...
      text('[data-queue-confirm-title]', `移除「${row.querySelector('[data-queue-title]').textContent}」后：`);
  ...
  on($('[data-queue-add]'), 'click', () => {
    const track = getVisibleTracks().find(item => item.id === player.snapshot().activeTrackId);
