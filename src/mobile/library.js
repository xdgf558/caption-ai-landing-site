...
export async function personalMusic(request,env,db,s,now) {
 ...
 else if(isListen){requireValue(request.method==='POST');exactKeys(body,['trackId','audioVersion','variant','eventId','historyEpoch','audibleSeconds','positionSeconds']);
 ...
 if(isFavorite){
  ...
  result={accepted:true};writes.push(db.prepare('INSERT INTO mobile_music_recent(account_id,track_id,played_at,position) VALUES(?,?,?,?) ON CONFLICT(account_id,track_id) DO UPDATE SET played_at=excluded.played_at,position=excluded.position').bind(account,body.trackId,now,body.positionSeconds),
 ...
 ]);}catch(error){await principal(db,request,Date.now());const r=await replay();if(r)return r;const latest=await db.prepare('SELECT revision FROM mobile_music_state WHERE account_id=?').bind(account).first();if(latest.revision!==state.revision)throw new MobileError('VERSION_CONFLICT',409);throw error;}
 return result;
}
